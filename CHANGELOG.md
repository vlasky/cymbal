...
- **`cymbal changed` — diff-scoped impact in one call** — maps a git diff to the symbols it touches and reports each one's references and transitive impact, so reviewing "what does this change affect?" is a single command instead of parsing the diff and running `impact` per symbol. Defaults to the working tree vs `HEAD`; `--staged` diffs the index, `--base <ref>` diffs the working tree against another ref (e.g. your branch point). Changed symbols are attributed by overlapping the diff's changed lines (not whole hunks) with indexed symbol ranges, mapping each line to its enclosing navigable definition (function/method/type), so function-local variables aren't mis-reported. Per symbol it prints exact, un-truncated `references` counts (`reference_rows` / `referencing_files`, split production/test/unknown) and a capped `impact` caller summary with its own `truncated` flag. Bounded by `--max-symbols` (default 40) and `--max-impact` total caller rows (default 500), with `--limit`/`--depth` per symbol and `--no-tests` passthrough; deleted files are reported, not silently dropped. Impact and references are name-scoped (cymbal resolves by name), so counts for a name with several definitions may span them. Operates only on the current worktree; arbitrary commit ranges whose new side isn't the working tree are unsupported.
